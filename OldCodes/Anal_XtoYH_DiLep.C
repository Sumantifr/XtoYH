...
int main(int argc, char *argv[])
{
   ...
   */
   ...
   Tout->Branch("HTlep_pt_JES_up", &HTlep_pt_JES_up, "HTlep_pt_JES_up/F");
   Tout->Branch("HTlep_pt_JES_dn", &HTlep_pt_JES_dn, "HTlep_pt_JES_dn/F");
   Tout->Branch("HTlep_pt_JER_up", &HTlep_pt_JER_up, "HTlep_pt_JER_up/F");
   Tout->Branch("HTlep_pt_JER_dn", &HTlep_pt_JER_dn, "HTlep_pt_JER_dn/F");
   
   Tout->Branch("ST", &ST, "ST/F");
   Tout->Branch("ST_JES_up", &ST_JES_up, "ST_JES_up/F");
   Tout->Branch("ST_JES_dn", &ST_JES_dn, "ST_JES_dn/F");
   Tout->Branch("ST_JER_up", &ST_JER_up, "ST_JER_up/F");
   Tout->Branch("ST_JER_dn", &ST_JER_dn, "ST_JER_dn/F");
   ...
   */
   ...
   for (int ij=0; ij<nentries; ij++) {
	...
	if(isFastSIM){
		...
		anytrig_pass = true; 
		if(((int)vmuons.size())>1 && vmuons[0].pt>40 && vmuons[1].pt>30)  { trig_threshold_pass = true; trig_matching_pass = true; muon_trig_pass = true; }
		else if(((int)velectrons.size())>1 && velectrons[0].pt>28 && velectrons[1].pt>28)  { trig_threshold_pass = true; trig_matching_pass = true; electron_trig_pass = true; }
		else if(((int)vmuons.size())>0 && (int)velectrons.size()>0 && ((vmuons[0].pt>40 && velectrons[0].pt>30)||(vmuons[0].pt>30 && velectrons[0].pt>40))) { trig_threshold_pass = true; trig_matching_pass = true; emucross_trig_pass = true; }
    ...
    if(isMC && anytrig_pass && trig_threshold_pass && trig_matching_pass && !(muon_trig_pass && electron_trig_pass && emucross_trig_pass))
    {
    ...
    */
    ...
    if(Y_cand==-1 || W_cand[0]==-1 || W_cand[1]==-1) continue;
	*/
...
*/	
    ...
    if(vleptons.size()>1){
	 ...
	 for(int ilep=0; ilep<2; ilep++){	
		...
		else if (vleptons[0].indexemu>=0 && vleptons[ilep].indexemu<(velectrons.size())){
		...
		l_genindex[ilep] = (isMC)?get_nearest_Parton(genleps,vleptons[ilep].p4,0.4):-1;
    ...
    */
    if(Y_cand>=0) {
		...
		if(vleptons.size()>1){
			for(int ilep=0; ilep<2; ilep++){
				dR_lY[ilep] = delta2R(LJets[Y_cand].y,LJets[Y_cand].phi,vleptons[ilep].eta,vleptons[ilep].phi);
	...
	for(int jw=0; jw<2; jw++){
		...
		if(W_cand[jw]>=0) {
			...
			if(vleptons.size()>0 && pnu[jw].Eta()>-100){
				...
				H_JESup[jw] = (W_mom + vleptons[0].p4 + pnu[jw]).Pt()/H_mom.Pt();
				...
				H_JESdn[jw] = (W_mom + vleptons[0].p4 + pnu[jw]).Pt()/H_mom.Pt();
				...
				if(isMC){
					...
					H_JERup[jw] = (W_mom + vleptons[0].p4 + pnu[jw]).Pt()/H_mom.Pt();
					...
					H_JERdn[jw] = (W_mom + vleptons[0].p4 + pnu[jw]).Pt()/H_mom.Pt();
				}
	...
	*/
	...
	HTlep_pt_JES_up = (l_pt[0]+l_pt[1]+Y_pt*Y_JESup)*1./(HTlep_pt);
	HTlep_pt_JES_dn = (l_pt[0]+l_pt[1]+Y_pt*Y_JESdn)*1./(HTlep_pt);
	HTlep_pt_JER_up = (l_pt[0]+l_pt[1]+Y_pt*Y_JERup)*1./(HTlep_pt);
	HTlep_pt_JER_dn = (l_pt[0]+l_pt[1]+Y_pt*Y_JERdn)*1./(HTlep_pt);
	
	ST = l_pt[0]+l_pt[1]+MET_pt+Y_pt;
	ST_JES_up = (l_pt[0]+l_pt[1]+MET_pt_JESup+Y_pt*Y_JESup)*1./(ST);
	ST_JES_dn = (l_pt[0]+l_pt[1]+MET_pt_JESdn+Y_pt*Y_JESdn)*1./(ST);
	ST_JER_up = (l_pt[0]+l_pt[1]+MET_pt_JERup+Y_pt*Y_JERup)*1./(ST);
	ST_JER_dn = (l_pt[0]+l_pt[1]+MET_pt_JERdn+Y_pt*Y_JERdn)*1./(ST);
