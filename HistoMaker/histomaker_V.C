 ...
 for (int ii=0;ii<nproc;ii++)
  {
   ...
   TFile* final_file = TFile::Open("Histograms_Y_T_W_T_CR_V/Histogram_"+proc_Name[ii], "RECREATE");  
  ...
  TString sysnames[] = {"JES","JER","PU","LeptonSF","Prefire","PNbbSF","PNWSF"}; 
   ...
   for(int jk=0; jk<nbcat; jk++){
		...
		for(int lm=0; lm<nlid; lm++){
			...
			for(int isys=0; isys<nsys; isys++){
				...
				if(isys==3){
					// fix for lepton SF (needs to be corrected during remaking of small ntuples) //
					if(abs(l_pdgId)==13){
						if(leptonsf_weight_stat<1.e-6) { leptonsf_weight_stat = leptonsf_weight; }
						if(leptonsf_weight_syst<1.e-6) { leptonsf_weight_syst = leptonsf_weight; }
					}
					// end of fix //
